...
public class GitCommitService {
     ...
     * 获取最近N天的提交记录
     */
    public List<CommitInfo> getRecentCommits(String repositoryPath, int days) {
     ...
     * 获取提交统计信息
     */
    public String getCommitStatistics(List<CommitInfo> commits) {
        if (commits.isEmpty()) {
            return "无提交记录";
        }
        int totalCommits = commits.size();

        return String.format("提交次数：%d",
            totalCommits);
    }
}
